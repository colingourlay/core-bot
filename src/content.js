...
function resolveExternalLink(content, url) {
  if (url.match(GIF_URL_PATTERNS.GFYCAT) || url.match(GIF_URL_PATTERNS.GIPHY)) {
...
export function parseContent(el) {
  ...
  if (soleLinkEl) {
    resolveExternalLink(content, soleLinkHref);
  ...
  fetchOne(content.props.cmid, (err, doc) => {
    ...
    switch (doc.docType) {
      case 'Article':
        ...
        break;
      case 'ExternalLink':
        resolveExternalLink(content, doc.canonicalURL);
        break;
      case 'Image':
      case 'ImageProxy':
      case 'CustomImage':
