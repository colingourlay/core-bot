...
function formatEmoji(markup, emojiUsed) {
  ...
  if (matchedEmoji) {
    ...
    // console.log(markup.match(EMOJI_IMAGE_URLS_PATTERN));
    // markup.match(EMOJI_IMAGE_URLS_PATTERN).forEach(result => console.log(result.match(EMOJI_IMAGE_URLS_PATTERN)[0]));
    // markup.match(EMOJI_IMAGE_URLS_PATTERN).forEach(result => emojiUsed.add(imageURL));
  }
